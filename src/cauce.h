...
public:
  ...
  // compila un shader y lo adjunta al objeto programa 'id_prog' (que debe ser
  // >0)
  //
  // @param shader_type        (GLenum) one of: GL_VERTEX_SHADER,
  // GL_GEOMETRY_SHADER, GL_FRAGMENT_SHADER
  // @param shader_description (const char *) text description for error log
  // ('vertex shader', 'fragment shader', etc...)
  // @param shader_source      (const char *) source string
  //
  GLuint compilarAdjuntarShader(GLenum shader_type,
                                const char *shader_description,
  ...
  // usar (activar) el objeto programa para las siguientes operaciones de
  // visualización
  void activar();

  // Fija el color actual (valor por defecto del aributo de color)
  // @param nuevo_color (glm::vec3) nuevo color para visualizaciones sin tabla
  // de colores.
  //
  ...
  // pushes a copy of the current modelview matrix on top of the modelview
  // matrix stack
  void pushMM();
  ...
  // removes the current matrix on top of the modelview matrix stack (cannot be
  // empty)
  void popMM();
...
protected: // ---------------------------
  ...
  GLuint id_prog = 0, // nombre o identificador del objeto programa
      id_frag_shader =
          0, // nombre o identificador del objeto shader (fragment shader)
      id_vert_shader =
          0; // nombre o identificador del objeto shader (vertex shader)

  // variables estáticas con información del log errores
  static constexpr GLsizei log_long_max =
      1024 * 16; //  longitud máxima en chars del buffer para log
  ...
  glm::mat4 mat_modelview = glm::mat4(
      1.0); // current modelview matrix (initially equal to the identity matrix)
  std::vector<glm::mat4>
      pila_mat_modelview;       // stack for saved modelview matrices
  GLint loc_mat_modelview = -1; // uniform location for the modelview matrix

  glm::mat4 mat_proyeccion =
      glm::mat4(1.0); // current projection matrix (initially equal to the
                      // identity matrix)
  GLint loc_mat_proyeccion = -1; // uniform location for projection matrix
};

// returns a string with the name of a type from its corresponding OpenGL
// symbolic constant
