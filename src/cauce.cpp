...
void Cauce::imprimeInfoUniforms() {
  ...
  for (int i = 0; i < n_uniforms; i++) {
    ...
    glGetActiveUniform(id_prog, (GLuint)i, log_long_max, &log_long, &n_entradas,
                       &tipo, log_buffer);
    cout << "   Uniform " << i << ": " << log_buffer << " ("
         << NombreTipoGL(tipo) << " x" << n_entradas << ")." << endl;
  }
  assert(glGetError() == GL_NO_ERROR);
}
...
GLuint Cauce::compilarAdjuntarShader(
    GLenum shader_type, // uno de GL_VERTEX_SHADER, GL_GEOMETRY_SHADER,
                        // GL_FRAGMENT_SHADER
    const char
        *shader_description,  // texto descriptivo por si hay error ('vertex
                              // shader', 'fragment shader', etc...)
    const char *shader_source // código fuente del shader
) {
  using namespace std;
  assert(shader_type == GL_VERTEX_SHADER || shader_type == GL_GEOMETRY_SHADER ||
...
GLint Cauce::leerLocation(const char *name) {
  ...
  if (location == -1)
    cout << "Warning: uniform '" << name << "' is not declared or not used."
         << endl;
...
void Cauce::crearObjetoPrograma() {
  ...
  assert(id_prog > 0);
  id_frag_shader = compilarAdjuntarShader(GL_VERTEX_SHADER, "vertex shader",
                                          fuente_vertex_shader);
  id_vert_shader = compilarAdjuntarShader(GL_FRAGMENT_SHADER, "fragment shader",
                                          fuente_fragment_shader);
...
void Cauce::pushColor() { pila_colores.push_back(color); }
// -----------------------------------------------------------------------------
...
void Cauce::fijarMatrizProyeccion(const glm::mat4 &new_projection_mat) {
  ...
  glUniformMatrix4fv(loc_mat_proyeccion, 1, GL_FALSE,
                     glm::value_ptr(mat_proyeccion));
  assert(glGetError() == GL_NO_ERROR);
}
// ---------------------------------------------------------------------------------------------
...
void Cauce::resetMM() {
  ...
  glUniformMatrix4fv(loc_mat_modelview, 1, GL_FALSE,
                     glm::value_ptr(mat_modelview));
  assert(glGetError() == GL_NO_ERROR);
}
// ---------------------------------------------------------------------------------------------

void Cauce::pushMM() { pila_mat_modelview.push_back(mat_modelview); }
// ---------------------------------------------------------------------------------------------
