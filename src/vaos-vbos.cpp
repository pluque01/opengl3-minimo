...
// ------------------------------------------------------------------------------------------------------
// devuelve el tamaño en bytes de un valor a partir de entero asociado con el
// tipo del valor en OpenGL
...
// ------------------------------------------------------------------------------------------------------
// comprueba que el modo es válido para las llamadas glDrawArrays y
// glDrawElements
...
DescrVBOAtribs::DescrVBOAtribs(const unsigned p_index, const GLenum p_type,
                               const unsigned p_size,
                               const unsigned long p_count,
                               const void *p_data) {
...
DescrVBOAtribs::DescrVBOAtribs(const unsigned p_index,
...
void DescrVBOAtribs::crearVBO() {
  ...
  // fija este buffer como buffer 'activo' actualmente en el 'target'
  // GL_ARRAY_BUFFER
  ...
  // indicar, para este índice de atributo, la localización y el formato de la
  // tabla en el buffer
...
DescrVBOInds::DescrVBOInds(const GLenum p_type, const GLsizei p_count,
...
DescrVAO::DescrVAO(const unsigned p_num_atribs,
                   DescrVBOAtribs *vbo_posiciones) {
  // comprobar precondiciones
  assert(0 < p_num_atribs);
  assert(vbo_posiciones !=
         nullptr); // no se permite puntero nulo, debe apuntar a algún objeto
  assert(vbo_posiciones->leerIndex() ==
         0); // solo se permite el atributo de posiciones (con índice 0)
  vbo_posiciones
      ->comprobar(); // comprobar que el vbo está en un estado correcto.
  ...
  // crear el vector con los flags de habilitado/deshabilitado (todos a 'true',
...
// Comprueba que el estado del VAO es correcto justo antes de añadir una tabla
// de atributos con índice 'index' (aborta si no)
//
void DescrVAO::check(const unsigned index) {
  assert(dvbo_atributo[0] !=
         nullptr);   // el VAO ya debe tener el VBO de posiciones.
  assert(0 < index); // no permite el índice 0, son las posiciones y se dan al
                     // construir el VAO
  assert(index < num_atribs); // no permite índices fuera de rango
  assert(dvbo_atributo[index] ==
         nullptr); // no permite añadir un atributo dos veces
  assert(array ==
         0); // no permite añadir atributos si el VAO ya esá alojado en la GPU
}
// ----------------------------------------------------------------------------
...
void DescrVAO::agregar(DescrVBOAtribs *p_dvbo_atributo) {
  ...
  assert(count == p_dvbo_atributo->leerCount()); // debe tener el mismo núm de
                                                 // items que el VBO posiciones

  // registrar el descriptor de VBO en la tabla de descriptores de VBOs de
  // atributos
...
void DescrVAO::agregar(DescrVBOInds *p_dvbo_indices) {
  // comprobar precondiciones:
  assert(p_dvbo_indices != nullptr); // no permite añadir atributos si el VAO ya
                                     // esá alojado en la GPU
...
// Crea el VAO en la GPU (solo se puede llamar una vez), deja el VAO activado
// como VAO actual. Crea y activa el VBO de posiciones y todos los VBOs de
// atributos que se hayan añadido. Si se ha añadido una tabla de índices, crea y
// activa el VBO de índices.
//
void DescrVAO::crearVAO() {
  CError();
  assert(
      array ==
      0); // asegurarnos que únicamente se invoca una vez para este descriptor
...
//
// @param index (unsigned) índice del atributo a habilitar o deshabilitar (no
// puede ser 0)
...
void DescrVAO::habilitarAtrib(const unsigned index, const bool habilitar) {
  ...
  assert(
      0 <
      index); // el atributo 0 siempre está habilitado, no puede deshabilitarse
  assert(index < num_atribs);              // al índice debe estar en su rango
  assert(dvbo_atributo[index] != nullptr); // no tiene sentido usarlo para un
                                           // atributo para el cual no hay tabla
...
//
// @param mode (GLenum) modo de visualización (GL_TRIANGLES, GL_LINES,
// GL_POINTS,  GL_LINE_STRIP o GL_LINE_LOOP)
//
void DescrVAO::draw(const GLenum mode) {
  ...
  // si el VAO no está creado, crearlo y dejarlo 'binded', si ya está creado,
  // solo se hace 'bind'
  if (array == 0)
    crearVAO();
  else
