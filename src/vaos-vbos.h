...
private:
  ...
  GLuint buffer =
      0; // nombre o id del buffer en la GPU (0 antes de crearlo, >0 después)
  ...
  GLsizeiptr tot_size =
      0; // tamaño completo de la tabla en bytes (=count*size*sizeof(c-type))

  const void *data = nullptr; // datos originales en la CPU (null antes de
                              // saberlos, no null después)
  void *own_data = nullptr;   // si no nulo, tiene copia de los datos (propiedad
                              // de este objeto).

  // Hace una copia de los datos de la tabla en una zona de memoria propiedad de
  // esta instancia (copia los datos originales en 'data' en 'own_data', solo
  // una vez).
  //
  void copiarDatos();
...
public:
  ...
  // Crea un descriptor de VBO de atributos, a partir de sus metadatos (type,
  // size, count) y un puntero a los datos (p_data)
  //
  ...
  // Crea un descriptor de VBO de atributos, a partir de una tabla de
  // coordenadas, almacenada como un vector (std::vector) de 'vec3'.
  //
  // @param p_index (unsigned)     índice del atributo
  ...
  // Crea un descriptor de VBO de atributos, a partir de una tabla de
  // coordenadas, almacenada como un vector (std::vector) de 'vec2'.
  //
  // @param p_index  (unsigned)     índice del atributo
  ...
  // Comprueba que los descriptores de la tabla de datos son correctos, aborta
  // si no
  //
  void comprobar() const;

  // Crea el VBO en la GPU (solo se puede llamar una vez), deja el VBO
  // habilitado en el índice de atributo, requiere que 'buffer' esté a cero
  // (evita llamarlo 2 veces) deja en buffer el identificador de VBO
  //
  void crearVBO();
  ...
  // Libera la memoria ocupada por el VBO, tanto en la memoria de la aplicación,
  // como en la memoria del buffer en la GPU (si ya se ha creado)
  //
  ~DescrVBOAtribs();
};
...
private:
  ...
  GLuint buffer =
      0; // nombre o id del buffer en la GPU (0 antes de crearlo, >0 después)
  GLenum type = 0; // tipo de los valores (GL_UNSIGNED_BYTE, GL_UNSIGNED_SHORT,
                   // GL_UNSIGNED_INT)
  GLsizei count = 0; // número de índices en la tabla (>0)
  GLsizeiptr tot_size =
      0; // tamaño completo de la tabla en bytes (=count*sizeof(c-type))

  const void *indices = nullptr; // datos originales en la CPU (null antes de
                                 // saberlos, no null después)
  void *own_indices = nullptr;   // si no nulo, tiene copia de los datos
...
public:
  ...
  // Crea un descriptor de VBO de índices, a partir de sus metadatos (type,
  // count) y un puntero a los datos (p_indices)
  //
  // @param p_type  (GLEnum)   tipo de los datos (GL_UNSIGNED_BYTE,
  // GL_UNSIGNED_SHORT o GL_UNSIGNED_INT)
  // @param p_count (unsigned) número de índices (>0)
  // @param p_data  (void *)   puntero al array de índices (no nulo, solo se
  // lee)
  //
  DescrVBOInds(const GLenum p_type, const GLsizei p_count, const void *p_data);

  // Crea un descriptor de VBO de índices, a partir de una tabla de tuplas con 3
  // enteros (se puede usar típicamente para tablas de triángulos)
  //
  // @param src_vec  (vector<uvec3> &)  tabla de tuplas de 3 enteros
  // (triángulos)
  //
  DescrVBOInds(const std::vector<unsigned> &src_vec);

  // Crea un descriptor de VBO de índices, a partir de una tabla de tuplas con 3
  // enteros (se puede usará típicamente para tablas de triángulos)
  //
  // @param src_vec  (vector<uvec3> &)  tabla de tuplas de 3 enteros
  // (triángulos)
  //
  ...
  //   1. Crea el VBO y envía la tabla de índices a la GPU (únicamente la
  //   primera vez)
  //   2. Hace 'bind' de la tabla en el 'target' GL_ELEMENT_ARRAY_BUFFER
  //
  void crearVBO();

  // destruye un VAO (libera memoria dinámica de la aplic. y elimina VAO en la
  // GPU)
  ~DescrVBOInds();
};
...
private:
  ...
  void check(const unsigned index); // comprueba precondiciones antes de añadir
                                    // tabla de atribs
...
public:
  ...
  // Crea un descriptor de VAO, dando un descriptor del VBO de posiciones de
  // vértices
  //
  // @param p_num_atribs     (unsigned)        número de atributos que puede
  // tener este VAO
  // @param dvbo_posiciones  (DescrVBOAttrib *) puntero al descriptor del VBO de
  // atributos (no nulo)
  //
  DescrVAO(const unsigned p_num_atribs, DescrVBOAtribs *vbo_posiciones);

  // Crea el identificador del VAO (array), crea los identificadores de los VBOs
  // asociados, transfiere los datos de todas las tablas a la GPU.
  //
  void crearVAO();

  // Añade un descriptor de VBO de atributos
  //
  // @param index (unsigned) índice del atributo (no puede ser 0, la tabla de
  // posiciones se da en el constructor)
  // @param vbo_atributo (DescrVBOAtribs *) puntero al descriptor (no puede ser
  // nulo)
  //
  void agregar(DescrVBOAtribs *vbo_atributo);

  // Añade el descriptor de índices, por tanto el VAO pasa a ser indexado.
  // @param vbo_indices (DescrVBOInds *) puntero al descriptor del VBO de
  // índices (no puede ser nulo)
  //
